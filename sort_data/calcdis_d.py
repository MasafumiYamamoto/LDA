def main(rootpath_,tnum_,bnum_,rflag_,tflag_):
	...
	ifile=open(pas+"subrev_1000_b"+str(bnum)+"t"+str(tnum)+"_LSI.csv","r")
	...
	if(rflag!=1):
		sfile=open(pas+"mergeLSI_nrnt_over4b"+str(bnum)+"t"+str(tnum)+".csv","r")
	elif(rflag_==1):
		sfile=open(pas+"mergeLSI_rnt_over4b"+str(bnum)+"t"+str(tnum)+".csv","r")
	...
	l=0
	if(rflag!=1):
		os.mkdir(pas+"resb"+str(bnum)+"t"+str(tnum)+"nrnt")
	elif(rflag==1):
		os.mkdir(pas+"resb"+str(bnum)+"t"+str(tnum)+"rnt")
	for line in ilist:
		l=l+1
		outfile=open(pas+"hoge.csv","wb")
		if(rflag!=1):
			outfile=open(pas+"resb"+str(bnum)+"t"+str(tnum)+"nrnt/"+line+".csv","wb")
		elif(rflag==1):
			outfile=open(pas+"resb"+str(bnum)+"t"+str(tnum)+"rnt/"+line+".csv","wb")
